...
export class DevTools {
  ...
  attachMonkeyPatchListeners() {
    ...
    document.addEventListener('keydown', (event: any) => {
      if (_self.UI && UI.KeyboardShortcut.eventHasCtrlOrMeta(event) && String.fromCharCode(event.which).toLowerCase() === 'r') {
        event.handled = true;
      }
    });
  }
  ...
  monkeypatchTimelineFeatures() {
    ...
    UI.inspectorView.showPanel('timeline').then(_ => {
      ...
      const formatTime = value => (Number as any).millisToString(value, true);
  ...
  monkepatchSetWindowPosition() {
    ...
    const plzRepeat = () => setTimeout(() => this.monkepatchSetWindowPosition(), 100);
  ...
  monkeyPatchRequestWindowTimes() {
    const viewerInstance = this.viewerInstance;
    const plzRepeat = () => setTimeout(() => this.monkeyPatchRequestWindowTimes(), 100);
  ...
  monkeyPatchWindowChanged() {
    const plzRepeat = () => setTimeout(() => this.monkeyPatchWindowChanged(), 100);
    if (typeof PerfUI === 'undefined' || typeof PerfUI.FlameChart === 'undefined' ) return plzRepeat();
  ...
  tweakUI() {
    try {
      ...
      (document.querySelector('.root-view .tabbed-pane')
        .shadowRoot.querySelector('.vbox > .tabbed-pane-header') as any)
        .style.display = 'none';
  ...
  monkeyPatchingHandleDrop() {
    ...
    if (_window.Timeline && _window.Timeline.TimelinePanel) {
      const viewerInstance = this.viewerInstance;
