...
================================================================================
  ...
  REQUIRED SECTIONS (all must be present):
    1. H1 = "How to {accomplish goal}"  (goal-titled, imperative)
    2. Goal / when to use   (one or two sentences)
    3. Steps                (ordered, minimal, real-world; assume competence)
    4. Result               (how to know it worked)
    5. See also             (one-line links: the REFERENCE for the symbols used +
                             the CONCEPT it assumes — MANDATORY, every How-to links
                             its assumed Concept — ADR-RXA-0004 Consequences)

  FORBIDDEN SECTIONS (SPLIT / link out if you have this content):
    - NO teaching / first-principles → that is a TUTORIAL.
    - NO full API dump               → link the REFERENCE (a one-liner, never the table).
    - NO long "why" essays           → link the CONCEPT.
