...
> `RxActions` is a powerful yet simple tool to manage actions of any type.
...
- ✅ Dispatch actions
- ✅ Transformations help you to have a clean template
- ✅ Fully Typed
...
`RxActions` are instantiated by the creation function `rxActions` coming from the `@rx-angular/state/actions` package.
