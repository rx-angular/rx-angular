---
id: set-up-rebundle
title: 'How to set up rebundle'
diataxis_type: how-to
package: rebundle
legacy_guard: false
sidebar_label: 'Set up rebundle'
sidebar_position: 1
...
rebundles chunks in memory before they are written to disk. This avoids
expensive re-transpilation.
...
[`@angular-builders/custom-esbuild`](https://github.com/just-jeb/angular-builders/tree/master/packages/custom-esbuild#custom-esbuild-application).
...
2. Change the application's `build` target to
...
[Configure merge strategies](./configure-merge-strategies.md).
...
The configure generator installs any required dependencies and updates the
selected project's build target so the chunk optimizer is registered as an
esbuild plugin.
