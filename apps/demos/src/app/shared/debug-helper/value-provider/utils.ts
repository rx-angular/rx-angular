...
export function toRandomItems(ids: number[]): TestItem[] {
  ...
  return new Array(ids.length)
    .fill(0)
    .map((v) => ({ id: _ids.pop(), value: toRandom() }));
}

export function toNewItems(
  arr: TestItem[] = [],
  numItems: number,
  maxId = theMax,
): TestItem[] {
...
export function updateItemMutable(arr = [], itemIds: number[]) {
  ...
  getItems(arr, itemIds).forEach(
    (i) => (arr.find((ii) => i.id === ii.id).value = toRandom()),
  );
  return arr;
}
...
export function updateItemImmutable(arr = [], num: number) {
  ...
  return [
    ...updateItemMutable(arr, itemIds).map((i) =>
      itemIds.find((id) => id + '' === i.id + '') ? { ...i } : i,
    ),
  ];
}
...
export function prependItemMutable(arr = [], numItems: number) {
  const newItems = toNewItems(arr, numItems);
  return [...newItems, ...arr];
}
...
export function prependItemImmutable(arr = [], numItems: number) {
  return [...prependItemMutable(arr, numItems)];
}
...
export function moveItemsImmutable(
  arr: TestItem[] = [],
  num: number,
): TestItem[] {
...
export function removeItemsImmutable(arr: TestItem[] = [], num: number) {
  return [
    ...removeItemsMutable(
      arr,
      getRandomItems(arr, num).map((i) => i.id),
    ),
  ];
}
...
export class GliphyApi {
  ...
  search(queryParams: { q: string; [key: string]: any }) {
    ...
    return from(fromFetch(this.getUrl(['search'], queryParams))).pipe(
      this.handleFetch,
      share(),
    );
  }
