...
import { groupBy, map, mergeAll, mergeMap, switchMap, tap } from 'rxjs/operators';
  ...
  function _leRender() {
    return (o$: Observable<IterableChanges<T>>) => o$.pipe(
      map((change) => {
        ...
        if (insertions.length + removals.length > 0) {
          _leWork.set(-1, [() => {
            config.cdRef.detectChanges();
            return null;
          }]);
        }
        return _leWork.entries();
      }),
      switchMap(workEntries => {
        return combineLatest([
          ...Array.from(workEntries).map(e => {
            return reactSchedulerTick([PriorityNameToLevel.normal, () => {
              let ev;
