...
(window as any).__postTaskScheduler__present =
  typeof window !== 'undefined' ? (window as any).scheduler : false;

export const postTaskScheduler: PostTaskScheduler =
  typeof window !== 'undefined'
    ? ((window as any).scheduler
        ? (getZoneUnPatchedApi('scheduler') as PostTaskScheduler)
        : false) ||
      ({
        postTask<T>(
          cb: () => T,
          ...
          return new Promise<T>((resolve) => {
            setTimeout(
              function () {
                console.warn(
                  'postTask not implemented. Use setTimeout scheduled over Promise as fallback',
                );
                resolve(cb());
              },
    ...
    : {
        postTask: <T>(
          cb: () => void,
          options: SchedulerPostTaskOptions,
        ): Promise<T> => Promise.resolve() as unknown as Promise<T>,
      };
