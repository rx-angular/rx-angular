...
@Component({
    ...
    template: `
        ...
        <button (click)="nextConfigSubject.next('pessimistic1')">pessimistic1</button>
...
export class DraftParent01Component implements AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit(): void {
        ...
        this.subscription.add(
            this.cfgService.select('strategy')
                .subscribe(name => this.cdAware.nextConfig(name))
        );

        this.subscription.add(
            this.nextValueSubject.subscribe(
                next => {
                    let v: any = EMPTY;
                    if (next === 'SINGLE') {
                        v = of(Math.random());
        ...
        this.subscription.add(
            this.nextConfigSubject.subscribe(
                name => {
