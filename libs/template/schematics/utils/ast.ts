...
function addSymbolToNgModuleMetadata(
  ...
  if (matchingProperties.length === 0) {
    ...
    if (expr.properties.length === 0) {
      pos = expr.getEnd() - 1;
      insert = `  ${metadataField}: [${symbolName}]\n`;
    } else {
      ...
      if (matches && matches.length > 0) {
        insert = `,${matches[0]}${metadataField}: [${symbolName}]`;
      } else {
        insert = `, ${metadataField}: [${symbolName}]`;
      }
    }
    if (importPath !== null) {
      return [
        new InsertChange(ngModulePath, pos, insert),
      ];
    } else {
      return [new InsertChange(ngModulePath, pos, insert)];
    }
  }
