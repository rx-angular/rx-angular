// Source rxjs: https://github.com/ReactiveX/rxjs/blob/6.5.4/src/internal/observable/fromEvent.ts
import { Observable, Subscriber } from 'rxjs';
import { getZoneUnPatchedApi } from '../../../core/utils/make-zone-less';
...
function setupSubscription<T>(
  ...
  if (isEventTarget(sourceObj)) {
    ...
    getZoneUnPatchedApi('addEventListener', source)(
      eventName,
      handler,
      options
    );
    ...
    const source = sourceObj;
    getZoneUnPatchedApi('on', sourceObj)(eventName, handler);
    unsubscribe = () => getZoneUnPatchedApi('off', source)(eventName, handler);
  } else if (isNodeStyleEventEmitter(sourceObj)) {
    const source = sourceObj;
    getZoneUnPatchedApi('addListener', sourceObj)(
      eventName,
      handler as NodeEventHandler
    );
    unsubscribe = () =>
      getZoneUnPatchedApi('removeListener', source)(
        eventName,
        handler as NodeEventHandler
      );
